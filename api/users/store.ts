const VALID_RANGES = ['24H', '7D', '30D', '90D', 'YTD'] as const;
const DEFAULT_RANGE = '30D';

type Range = typeof VALID_RANGES[number];

const SCALE: Record<Range, { dau: number; users: number; revenue: number; matches: number }> = {
...
const GEO_COUNTRIES = [
  { country: 'United States', base: 82400, pct: 28.9 },
  { country: 'Germany',       base: 41200, pct: 14.5 },
  { country: 'Japan',         base: 38700, pct: 13.6 },
  { country: 'Brazil',        base: 29100, pct: 10.2 },
  { country: 'India',         base: 24800, pct:  8.7 },
  { country: 'Other',         base: 68712, pct: 24.1 },
];
...
const sin = (i: number, amp: number, freq: number, phase = 0) => Math.round(Math.sin(i * freq + phase) * amp);
const fmt = (n: number) => n.toLocaleString('en-US');

export const parseRange = (query: Record<string, unknown>): Range => {
  const r = ((query.range as string) || DEFAULT_RANGE).toString().toUpperCase();
  return (VALID_RANGES.includes(r as Range) ? r : DEFAULT_RANGE) as Range;
};
...
const getLabels = (range: Range): string[] => {
  switch (range) {
...
export const getUserActivity = (range: Range) => {
...
export const getUserGrowth = (range: Range) => {
  const { dau, users } = SCALE[range];
...
export const getDeviceSplit = (range: Range) => {
...
export const getGeoData = (range: Range) => {
  const { dau } = SCALE[range];
